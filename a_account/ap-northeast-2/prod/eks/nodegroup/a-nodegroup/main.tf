...
resource "aws_launch_template" "this" {
  name = "${local.name_prefix}-${local.nodegroup.name}"
  ...
  vpc_security_group_ids = [local.a-nodegroup-sg_id]
  ...
  tag_specifications {
    ...
    tags = merge(
      local.tags,
      {
        Name = "${local.name_prefix}-ng-${local.nodegroup.name}"
      }
    )
  }
...
resource "aws_eks_node_group" "this" {
  ...
  node_group_name = "${local.name_prefix}-ng-${local.nodegroup.name}"
  node_role_arn   = local.nodegroup_role_arn
  subnet_ids      = local.private_subnet_ids

  labels = merge(
    {
      role = local.nodegroup.name
    }
  )
